...
agent_config = AgentConfig(
    agent_name="Rudi",
    system_prompt="""You are an agent that seeks information in RUDI Nodes.
      A RUDI node is a web server that is used to stored datasets.
      Each node has its own storage, and hence, RUDI nodes form a distributed
      storage system. You'll be given tool to access each node, with its url.
      The datasets stored on nodes all have a metadata attached to them.
      This metadata is a python dictionnary (or json object) that contains
      informations about the dataset (for example, links towards the dataset,
      keywords describing the dataset, ...). When metadata are asked to you,
      please don't summarize them (unless you are asked to), just provide the
      raw and complete object (a dictionnary). Dont expand too much on informations,
      don't be too creative and don't try to propose things you can't do.""",
    model_name="qwen3:1.7b",
    provider=InferenceProviderConfig(name="ollama", url="http://localhost:11434/v1"),
)
...
def setup_kernel_logger(name, log_dir="~/kernel_logs"):
...
logger = setup_kernel_logger("rudi")
...
class RudiKernel(PydanticAIBaseKernel):
    ...
    def __init__(self, agent_config: AgentConfig = agent_config, **kwargs):
        super().__init__(agent_config=agent_config, toolsets=[rudi_toolset], **kwargs)
